    ...
    <Card :bordered="false">
      ...
      <Row>
       <Col span="24" style="margin-top: 15px;">
        ...
        <Page :total="count" ref="currents" show-elevator show-total @on-change="page"  />
       </Col>
      </Row>
      <Modal
        v-model="modal1"
...
<script>
  import {getList,megetList, setArticle, searchArticle,mesearchArticle} from '@/api/article'
  ...
  created(){
      ...
      getList(getToken('token')).then((res)=>{
        ...
        for (let i=0;i<item.length;i++){
          ...
          this.data.push({
            'title':item[i].title,
            'create_time':item[i].create_time,
  ...
  methods: {
    ...
    search(){
      if(this.viewAccessSuper){
        searchArticle(this.selected?this.selected:'',this.title,getToken('token')).then(res=>{
     ...
     if(this.viewAccessAll){
       mesearchArticle(this.selected?this.selected:'',this.title,getToken('token')).then(res=>{
         this.$refs.currents.currentPage=1
         this.count=res.data.count
         this.data = []
         let item = res.data.results
